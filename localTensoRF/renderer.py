...
def render(
    ...
    if test:
        ...
        is_test_id = [
            fbase in test_dataset.all_fbases for fbase in train_dataset.all_fbases
        ]
    ...
    for i, idx in tqdm(enumerate(idxs)):
        ...
        if test and add_frame_to_list:
            ...
            if test_dataset.all_fwd_flow is not None:
                ...
                pred_fwd_flow = (
                    get_pred_flow(
                        pts, ij, fwd_cam2cams, local_tensorfs.focal(W), center
                    )
                    .cpu()
                    .numpy()
                )
                pred_bwd_flow = (
                    get_pred_flow(
                        pts, ij, bwd_cam2cams, local_tensorfs.focal(W), center
                    )
                    .cpu()
                    .numpy()
                )
                pred_fwd_flow, pred_bwd_flow = (
                    pred_fwd_flow.reshape(H, W, 2),
                    pred_bwd_flow.reshape(H, W, 2),
                )
                fwd_flow = cv2.resize(
                    test_dataset.all_fwd_flow[test_dataset.all_fbases[fbase]],
                    (W, H),
                    interpolation=cv2.INTER_NEAREST,
                )
                fwd_mask = cv2.resize(
                    test_dataset.all_fwd_mask[test_dataset.all_fbases[fbase]],
                    (W, H),
                    interpolation=cv2.INTER_NEAREST,
                )
                bwd_flow = cv2.resize(
                    test_dataset.all_bwd_flow[test_dataset.all_fbases[fbase]],
                    (W, H),
                    interpolation=cv2.INTER_NEAREST,
                )
                bwd_mask = cv2.resize(
                    test_dataset.all_bwd_mask[test_dataset.all_fbases[fbase]],
                    (W, H),
                    interpolation=cv2.INTER_NEAREST,
                )
                ...
                fwd_flow_err0 = (
                    np.abs(pred_fwd_flow[..., 0] - fwd_flow[..., 0]) * fwd_mask / W
                )
                ...
                fwd_flow_err1 = (
                    np.abs(pred_fwd_flow[..., 1] - fwd_flow[..., 1]) * fwd_mask / W
                )
                ...
                bwd_flow_err0 = (
                    np.abs(pred_bwd_flow[..., 0] - bwd_flow[..., 0]) * bwd_mask / W
                )
                ...
                bwd_flow_err1 = (
                    np.abs(pred_bwd_flow[..., 1] - bwd_flow[..., 1]) * bwd_mask / W
                )
                bwd_flow_cmp1 = np.vstack([bwd_flow_cmp1, bwd_flow_err1])
                bwd_flow_cmp = np.hstack([bwd_flow_cmp0, bwd_flow_cmp1])
                fwd_flow_cmp_tb.append(
                    torch.from_numpy(fwd_flow_cmp).clamp(0, 1)
                )  # Only need this one for TensorBoard
                bwd_flow_cmp_tb.append(
                    torch.from_numpy(bwd_flow_cmp).clamp(0, 1)
                )  # Only need this one for TensorBoard

            # Depth error
            if test_dataset.all_invdepths is not None:
                invdepths = torch.from_numpy(
                    cv2.resize(
                ...
                dyn_depth_norm, gt_depth_norm, depth_loss_arr = compute_depth_loss(
                    1 / depth_map[None].clamp(1e-6), invdepths
                )

                depth_cmp = torch.vstack(
                    [
                        0.5 * dyn_depth_norm[0].reshape(H, W),
                        0.5 * gt_depth_norm[0].reshape(H, W),
                        depth_loss_arr[0].reshape(H, W),
                    ]
                )
                depth_cmp_tb.append(
                    depth_cmp.clamp(0, 1)
        ...
        pose_vis = cv2.resize(
            pose_vis,
            (
                int(pose_vis.shape[1] * rgb_map.shape[0] / pose_vis.shape[0]),
                rgb_map.shape[0],
            ),
        )
        depth_map_vis = torch.permute(
            depth_map_vis.detach().cpu() * 255, [1, 2, 0]
        ).byte()
        ...
        if save_frames and savePath is not None:
            ...
            cv2.imwrite(
                f"{savePath}/rgb_maps/{fbase}.{img_format}",
                255 * rgb_map.numpy()[..., ::-1],
            )
            cv2.imwrite(
                f"{savePath}/rgb_maps/{fbase}_pose.{img_format}", pose_vis[..., ::-1]
            )
            cv2.imwrite(
                f"{savePath}/depth_maps/{fbase}.{img_format}",
                depth_map_vis.numpy()[..., ::-1],
            )
            if save_raw_depth:
                cv2.imwrite(
                    f"{savePath}/depth_maps/{fbase}.tiff", depth_map.cpu().numpy()
                )
